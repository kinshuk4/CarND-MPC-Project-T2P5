...
private:
    ...
    //weights for penalizing various errors
...
public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        fg[0] = 0;
        // Use above weights to minimize various errors and behaviours
        for (int i = 0; i < N; i++) {
            fg[0] += w_cte * CppAD::pow(vars[cte_start + i] - ref_cte, 2);
            fg[0] += w_epsi * CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
            fg[0] += w_dv * CppAD::pow(vars[v_start + i] - ref_v, 2);
        }
        ...
        for (int i = 0; i < N - 2; i++) {
            fg[0] += w_d_delta * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
            fg[0] += w_d_a * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
        }
